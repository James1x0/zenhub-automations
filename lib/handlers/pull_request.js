...
exports.opened = async function handleOpenedPR (tools) {
  ...
  if (REVIEW_COLUMN) {
    ...
    if (firstIssue) {
      try {
        await zh.issues.connectIssue(payload.repository.id, payload.number, payload.repository.id, firstIssue);
      } catch (e) {
        failures.push(`Failed to connect issue ${payload.number} to ${firstIssue}: ${e}`);
      }
    }
  }
